...
import { CheckCircle, ArrowRight, ChefHat, Zap, ShieldCheck, BookOpen, BarChart3, Utensils, MessageSquare } from 'lucide-react';
...
const TOOL_CARDS = [
  { icon: <Utensils className="h-7 w-7 text-indigo-600" />, bg: 'bg-indigo-50', border: 'border-indigo-200' },
  { icon: <BarChart3 className="h-7 w-7 text-blue-600" />, bg: 'bg-blue-50', border: 'border-blue-200' },
  { icon: <ShieldCheck className="h-7 w-7 text-emerald-600" />, bg: 'bg-emerald-50', border: 'border-emerald-200' },
  { icon: <BookOpen className="h-7 w-7 text-amber-600" />, bg: 'bg-amber-50', border: 'border-amber-200' },
  { icon: <MessageSquare className="h-7 w-7 text-pink-600" />, bg: 'bg-pink-50', border: 'border-pink-200' },
  { icon: <Zap className="h-7 w-7 text-orange-600" />, bg: 'bg-orange-50', border: 'border-orange-200' },
];
...
export default function ChatGPTRestaurantes() {
  ...
  const k = (key: string) => t(`landingChatGPT.${key}`, { returnObjects: true, userCount });
  const s = (key: string) => t(`landingChatGPT.${key}`, { userCount }) as string;
  ...
  const comparison = k('comparison.items') as Array<{ feature: string; chatgpt: string; aichef: string }>;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero */}
        <section className="relative overflow-hidden bg-gradient-to-br from-indigo-50 via-background to-blue-50/30 py-20 lg:py-32">
        ...
        {/* Resultados */}
        <section className="py-16 bg-gradient-to-r from-indigo-600/90 to-blue-500">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto text-center">
              {results.map((r, i) => (
                <div key={i}>
                  ...
                  <div className="text-sm text-indigo-100">{r.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-4xl mx-auto overflow-hidden rounded-2xl border shadow-lg">
              <div className="grid grid-cols-3 bg-indigo-600 text-white text-sm font-semibold">
                ...
                <div className="p-4 text-center border-l border-indigo-500">ChatGPT</div>
                <div className="p-4 text-center border-l border-indigo-500">AI Chef Pro</div>
              </div>
              {comparison.map((row, i) => (
                <div key={i} className={`grid grid-cols-3 text-sm ${i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                  ...
                  <div className="p-4 text-center text-muted-foreground border-l">{row.chatgpt}</div>
                  <div className="p-4 text-center text-indigo-700 font-medium border-l">{row.aichef}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <PricingV2 medium="landing-chatgpt" />
        ...
        {/* CTA final */}
        <section className="py-20 bg-gradient-to-br from-indigo-600 via-indigo-500 to-blue-500">
          <div className="container mx-auto px-4 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-white text-indigo-700 hover:bg-indigo-50" onClick={() => window.open(APP_URL, '_blank')}>
