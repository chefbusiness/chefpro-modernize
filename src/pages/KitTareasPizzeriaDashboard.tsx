...
// ── Template metadata ────────────────────────────────────────────
const TEMPLATES = [
  { key: 'apertura-cierre', icon: DoorOpen, title: 'Apertura y Cierre', desc: '6 checklists: apertura y cierre de horno, línea, sala y barra.' },
  { key: 'horno', icon: Flame, title: 'Tareas del Horno', desc: 'Encendido, temperatura 400-450 °C, limpieza, gestión leña.' },
  { key: 'manager', icon: ClipboardList, title: 'Tareas del Manager', desc: 'Diario, semanal, mensual + handover de turno.' },
  { key: 'perfiles', icon: Users, title: 'Tareas por Perfil', desc: 'Pizzero, cocinero línea, encargado, camarero, repartidor.' },
  { key: 'periodicas', icon: CalendarDays, title: 'Semanales y Mensuales', desc: 'Limpieza profunda, FIFO, mantenimiento horno y amasadora.' },
...
export default function KitTareasPizzeriaDashboard() {
  const { token } = useAuth('kit-tareas-pizzeria-jwt');
  ...
  return (
    <>
      <Helmet>
        <title>Kit de Tareas Pizzería — Dashboard | AI Chef Pro</title>
      ...
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <header className="sticky top-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/10">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <a href="/kit-tareas-pizzeria" className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm">
              <ArrowLeft className="w-4 h-4" />
              Kit de Tareas Pizzería
            </a>
        ...
        <section className="py-12 md:py-16 px-4 text-center">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold text-white mb-3 mt-4">
            Kit de Tareas <span className="text-[#FFD700]">Pizzería</span>
          </h1>
          <div className="mb-3">
            <ProductVersionBadge productId="kit-tareas-pizzeria" />
          </div>
        ...
        <ProductChangelog productId="kit-tareas-pizzeria" />
        {/* ── Cross-sell banners ──────────────────────────────── */}
        ...
        <footer className="py-8 px-4 border-t border-white/10">
          <div className="max-w-4xl mx-auto text-center">
            <p className="text-gray-500 text-sm mb-2">
              © 2026 AI Chef Pro · Kit de Tareas Pizzería · Todos los derechos reservados
            </p>
