...
export default function KitTareasAccessGate() {
  ...
  useEffect(() => {
    ...
    fetch('/.netlify/functions/verify-purchase', {
      ...
      body: JSON.stringify({ sessionId, product: 'kit-tareas' }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.token) {
          localStorage.setItem('kit-tareas-jwt', data.token);
          setStatus('success');
          setTimeout(() => navigate('/kit-tareas-library'), 1500);
        } else {
          setStatus('error');
      ...
      <div className="max-w-md mx-auto text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold text-white mb-2">Verificando tu compra del Kit de Tareas Recurrentes...</h1>
        ...
        {status === 'error' && (
          <>
            ...
            <a href="/kit-tareas" className="text-[#FFD700] hover:underline">
              Volver al Kit de Tareas Recurrentes
            </a>
          </>
        )}
      </div>
