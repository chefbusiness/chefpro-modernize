...
export default function KitTareasCafeteriaAccessGate() {
  ...
  useEffect(() => {
    ...
    fetch('/.netlify/functions/verify-purchase', {
      ...
      body: JSON.stringify({ sessionId, product: 'kit-tareas-cafeteria' }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.token) {
          localStorage.setItem('kit-tareas-cafeteria-jwt', data.token);
          setStatus('success');
          setTimeout(() => navigate('/kit-tareas-cafeteria-library'), 1500);
        } else {
          setStatus('error');
      ...
      <div className="max-w-md mx-auto text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold text-white mb-2">Verificando tu compra del Kit de Tareas Cafetería...</h1>
        ...
        {status === 'error' && (
          <>
            ...
            <a href="/kit-tareas-cafeteria" className="text-[#FFD700] hover:underline">
              Volver al Kit de Tareas Cafetería
            </a>
          </>
        )}
      </div>
