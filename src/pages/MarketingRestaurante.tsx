...
import { CheckCircle, ArrowRight, ChefHat, Megaphone, Star, Camera, MessageSquare, TrendingUp, Share2 } from 'lucide-react';
...
const TOOL_CARDS = [
  ...
  { icon: <Camera className="h-7 w-7 text-pink-600" />, bg: 'bg-pink-50', border: 'border-pink-200' },
  { icon: <MessageSquare className="h-7 w-7 text-blue-600" />, bg: 'bg-blue-50', border: 'border-blue-200' },
  { icon: <TrendingUp className="h-7 w-7 text-emerald-600" />, bg: 'bg-emerald-50', border: 'border-emerald-200' },
  { icon: <Megaphone className="h-7 w-7 text-orange-600" />, bg: 'bg-orange-50', border: 'border-orange-200' },
];
...
export default function MarketingRestaurante() {
  ...
  const k = (key: string) => t(`landingMarketing.${key}`, { returnObjects: true, userCount });
  const s = (key: string) => t(`landingMarketing.${key}`, { userCount }) as string;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero */}
        <section className="relative overflow-hidden bg-gradient-to-br from-violet-50 via-background to-pink-50/30 py-20 lg:py-32">
        ...
        {/* Resultados */}
        <section className="py-16 bg-gradient-to-r from-violet-600/90 to-pink-500">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto text-center">
              {results.map((r, i) => (
                <div key={i}>
                  ...
                  <div className="text-sm text-violet-100">{r.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* CTA final */}
        <section className="py-20 bg-gradient-to-br from-violet-600 via-violet-500 to-pink-500">
          <div className="container mx-auto px-4 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-white text-violet-700 hover:bg-violet-50" onClick={() => window.open(APP_URL, '_blank')}>
