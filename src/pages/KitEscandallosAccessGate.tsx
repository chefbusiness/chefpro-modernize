...
export default function KitEscandallosAccessGate() {
    ...
    const verify = async () => {
      try {
        const body = checkoutToken
          ? { checkoutSessionId: checkoutToken, product: 'kit-escandallos' }
          : { existingJwt: jwtToken, product: 'kit-escandallos' };
        ...
        if (data.valid && data.jwt) {
          localStorage.setItem('kit-escandallos-jwt', data.jwt);
          navigate('/kit-escandallos-library', { replace: true });
        } else {
          setStatus('error');
        }
      } catch {
