...
export default function KitTareasPasteleriaAccessGate() {
  ...
  useEffect(() => {
    ...
    fetch('/.netlify/functions/verify-purchase', {
      ...
      body: JSON.stringify({ sessionId, product: 'kit-tareas-pasteleria' }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.token) {
          localStorage.setItem('kit-tareas-pasteleria-jwt', data.token);
          setStatus('success');
          setTimeout(() => navigate('/kit-tareas-pasteleria-library'), 1500);
        } else {
          setStatus('error');
      ...
      <div className="max-w-md mx-auto text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold text-white mb-2">Verificando tu compra del Kit de Tareas Pastelería...</h1>
        ...
        {status === 'error' && (
          <>
            ...
            <a href="/kit-tareas-pasteleria" className="text-[#FFD700] hover:underline">
              Volver al Kit de Tareas Pastelería
            </a>
          </>
        )}
      </div>
