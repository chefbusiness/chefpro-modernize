...
import {
  ...
  DoorOpen, Warehouse, ClipboardList, Users,
  CalendarDays, PartyPopper, FileEdit, Megaphone, Calendar,
  Tablet, Package, Building2, Wallet,
} from 'lucide-react';
...
// ── Template metadata ────────────────────────────────────────────
const TEMPLATES = [
  { key: 'apertura-cierre', icon: DoorOpen, title: 'Apertura y Cierre', desc: '6 checklists: apertura y cierre de estaciones, empaquetado, expedicion y riders.' },
  { key: 'partidas', icon: Warehouse, title: 'Estaciones de Produccion', desc: 'Mise en place multi-marca, equipos, temperaturas, packaging.' },
  { key: 'manager', icon: ClipboardList, title: 'Tareas del Manager', desc: 'Diario, semanal, mensual + handover de turno.' },
  { key: 'perfiles', icon: Users, title: 'Tareas por Perfil', desc: 'Cocinero multi-marca, empaquetador, gestor plataformas, encargado.' },
  { key: 'periodicas', icon: CalendarDays, title: 'Semanales y Mensuales', desc: 'Limpieza profunda, FIFO, mantenimiento equipos, packaging.' },
  { key: 'eventos', icon: PartyPopper, title: 'Eventos y Festivos', desc: 'Picos de demanda, festivos, saturacion plataformas.' },
  { key: 'personalizable', icon: FileEdit, title: 'Plantilla Personalizable', desc: '3 plantillas en blanco para crear las tuyas.' },
  { key: 'apertura-negocio', icon: Building2, title: 'Apertura y Cierre del Negocio', desc: 'Checklist para abrir y cerrar la dark kitchen.' },
  { key: 'apertura-caja', icon: Wallet, title: 'Apertura y Cierre de Caja', desc: 'Fondo de caja, arqueo, cuadre y depósito.' },
  { key: 'bonus-briefing', icon: Megaphone, title: 'BONUS: Briefing Servicio', desc: 'Plantilla de briefing pre-turno.' },
  { key: 'bonus-calendario', icon: Calendar, title: 'BONUS: Calendario Anual', desc: '17 fechas clave de hosteleria.' },
];

export default function KitTareasDarkKitchenDashboard() {
  const { token } = useAuth('kit-tareas-dark-kitchen-jwt');
  ...
  return (
    <>
      <Helmet>
        <title>Kit de Tareas Dark Kitchen — Dashboard | AI Chef Pro</title>
      ...
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <header className="sticky top-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/10">
          <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
            <a href="/kit-tareas-dark-kitchen" className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm">
              <ArrowLeft className="w-4 h-4" />
              Kit de Tareas Dark Kitchen
            </a>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-6 text-center">
              ...
              <p className="text-gray-400 text-sm">
                Descarga los archivos .xlsx y abrelos con tu programa favorito. Todas las formulas se mantienen.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="py-10 px-4 border-t border-white/10">
          <div className="max-w-3xl mx-auto text-center space-y-4">
            <p className="text-gray-400 text-sm mb-3">
              Completa tu toolkit de gestion hostelera
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <a
                href="/kit-tareas-hamburgueseria"
                className="inline-block px-6 py-3 border border-[#FFD700]/50 text-[#FFD700] font-bold rounded-xl hover:bg-[#FFD700]/10 transition-all text-sm"
              >
                Ver Kit Tareas Hamburgueseria — €12
              </a>
              <a
        ...
        <footer className="py-8 px-4 border-t border-white/10">
          <div className="max-w-4xl mx-auto text-center">
            <p className="text-gray-500 text-sm mb-2">
              © 2026 AI Chef Pro · Kit de Tareas Dark Kitchen · Todos los derechos reservados
            </p>
