...
export default function KitTareasDarkKitchenAccessGate() {
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      setStatus('error');
      setErrorMsg('No se encontro la sesion de pago.');
      return;
    }
    ...
    fetch('/.netlify/functions/verify-purchase', {
      ...
      body: JSON.stringify({ sessionId, product: 'kit-tareas-dark-kitchen' }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.token) {
          localStorage.setItem('kit-tareas-dark-kitchen-jwt', data.token);
          setStatus('success');
          setTimeout(() => navigate('/kit-tareas-dark-kitchen-library'), 1500);
        } else {
          setStatus('error');
      ...
      <div className="max-w-md mx-auto text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold text-white mb-2">Verificando tu compra del Kit de Tareas Dark Kitchen...</h1>
        ...
        {status === 'error' && (
          <>
            ...
            <a href="/kit-tareas-dark-kitchen" className="text-[#FFD700] hover:underline">
              Volver al Kit de Tareas Dark Kitchen
            </a>
          </>
        )}
      </div>
