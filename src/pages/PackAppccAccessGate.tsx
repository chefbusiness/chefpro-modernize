...
export default function PackAppccAccessGate() {
  ...
  useEffect(() => {
    ...
    fetch('/.netlify/functions/verify-purchase', {
      ...
      body: JSON.stringify({ sessionId, product: 'pack-appcc' }),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.token) {
          localStorage.setItem('pack-appcc-jwt', data.token);
          setStatus('success');
          setTimeout(() => navigate('/pack-appcc-library'), 1500);
        } else {
          setStatus('error');
      ...
      <div className="max-w-md mx-auto text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold text-white mb-2">Verificando tu compra del Pack APPCC...</h1>
        ...
        {status === 'error' && (
          <>
            ...
            <a href="/pack-appcc" className="text-[#FFD700] hover:underline">
              Volver al Pack APPCC
            </a>
          </>
        )}
      </div>
