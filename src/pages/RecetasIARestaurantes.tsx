...
import { CheckCircle, ArrowRight, ChefHat, Sparkles, Globe, Calendar, Scale, BookOpen, Leaf } from 'lucide-react';
...
const TOOL_CARDS = [
  ...
  { icon: <Calendar className="h-7 w-7 text-emerald-600" />, bg: 'bg-emerald-50', border: 'border-emerald-200' },
  { icon: <Scale className="h-7 w-7 text-blue-600" />, bg: 'bg-blue-50', border: 'border-blue-200' },
  { icon: <BookOpen className="h-7 w-7 text-indigo-600" />, bg: 'bg-indigo-50', border: 'border-indigo-200' },
  { icon: <Leaf className="h-7 w-7 text-green-600" />, bg: 'bg-green-50', border: 'border-green-200' },
];
...
export default function RecetasIARestaurantes() {
  ...
  const k = (key: string) => t(`landingRecetas.${key}`, { returnObjects: true, userCount });
  const s = (key: string) => t(`landingRecetas.${key}`, { userCount }) as string;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {/* Resultados */}
        <section className="py-16 bg-gradient-to-r from-amber-600/90 to-orange-500">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto text-center">
              {results.map((r, i) => (
                <div key={i}>
                  ...
                  <div className="text-sm text-amber-100">{r.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-muted/20">
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-4xl mx-auto overflow-hidden rounded-2xl border shadow-lg">
              <div className="grid grid-cols-3 bg-amber-600 text-white text-sm font-semibold">
                <div className="p-4">{s('comparison.col_feature')}</div>
                <div className="p-4 text-center border-l border-amber-500">{s('comparison.col_other')}</div>
                <div className="p-4 text-center border-l border-amber-500">AI Chef Pro</div>
              </div>
              {comparison.map((row, i) => (
                <div key={i} className={`grid grid-cols-3 text-sm ${i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                  ...
                  <div className="p-4 text-center text-amber-700 font-medium border-l">{row.aichef}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <PricingV2 medium="landing-recetas-ia" />
        ...
        <section className="py-20 bg-gradient-to-br from-amber-600 via-amber-500 to-orange-500">
          <div className="container mx-auto px-4 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-white text-amber-700 hover:bg-amber-50" onClick={() => window.open(APP_URL, '_blank')}>
