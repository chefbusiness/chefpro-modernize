...
import { CheckCircle, ArrowRight, ChefHat, Calculator, TrendingDown, DollarSign, BarChart2, RefreshCw, PieChart } from 'lucide-react';
...
const TOOL_CARDS = [
  { icon: <Calculator className="h-7 w-7 text-rose-600" />, bg: 'bg-rose-50', border: 'border-rose-200' },
  { icon: <TrendingDown className="h-7 w-7 text-red-600" />, bg: 'bg-red-50', border: 'border-red-200' },
  { icon: <DollarSign className="h-7 w-7 text-emerald-600" />, bg: 'bg-emerald-50', border: 'border-emerald-200' },
  { icon: <BarChart2 className="h-7 w-7 text-blue-600" />, bg: 'bg-blue-50', border: 'border-blue-200' },
  { icon: <RefreshCw className="h-7 w-7 text-amber-600" />, bg: 'bg-amber-50', border: 'border-amber-200' },
  { icon: <PieChart className="h-7 w-7 text-indigo-600" />, bg: 'bg-indigo-50', border: 'border-indigo-200' },
];
...
export default function EscandallosRestaurante() {
  ...
  const k = (key: string) => t(`landingEscandallos.${key}`, { returnObjects: true, userCount });
  const s = (key: string) => t(`landingEscandallos.${key}`, { userCount }) as string;
    ...
    <div className="min-h-screen bg-background">
      ...
      <Helmet>
        {Object.entries(LANG_SLUGS).map(([lang, slug]) => {
          const href = lang === 'es' ? `${SITE_URL}/${slug}` : `${SITE_URL}/${slug}`;
      ...
      <main>
        {/* Hero */}
        <section className="relative overflow-hidden bg-gradient-to-br from-rose-50 via-background to-red-50/30 py-20 lg:py-32">
        ...
        {/* Resultados */}
        <section className="py-16 bg-gradient-to-r from-rose-600/90 to-red-500">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto text-center">
              {results.map((r, i) => (
                <div key={i}>
                  ...
                  <div className="text-sm text-rose-100">{r.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-muted/20">
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-4xl mx-auto overflow-hidden rounded-2xl border shadow-lg">
              <div className="grid grid-cols-3 bg-rose-600 text-white text-sm font-semibold">
                <div className="p-4">{s('comparison.col_feature')}</div>
                <div className="p-4 text-center border-l border-rose-500">{s('comparison.col_other')}</div>
                <div className="p-4 text-center border-l border-rose-500">AI Chef Pro</div>
              </div>
              {comparison.map((row, i) => (
                <div key={i} className={`grid grid-cols-3 text-sm ${i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                  ...
                  <div className="p-4 text-center text-rose-700 font-medium border-l">{row.aichef}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <PricingV2 medium="landing-escandallos" />
        ...
        {/* CTA final */}
        <section className="py-20 bg-gradient-to-br from-rose-600 via-rose-500 to-red-500">
          <div className="container mx-auto px-4 text-center text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-white text-rose-700 hover:bg-rose-50" onClick={() => window.open(APP_URL, '_blank')}>
                {s('cta_section.primary')}
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button size="lg" variant="outline" className="border-white text-white bg-white/10 hover:bg-white/20" onClick={() => window.open(APP_URL, '_blank')}>
