...
## Estado 6-sep 03:20 — rediseño UX EN PRODUCCIÓN; réplica en espera del pago de prueba
- **Live en `aichef.pro/kit-tareas-cafeteria`** (PR #79 `b38f311` + `ad5bce5` + `a989f39` en `main`): hero con el bloque cripto INTEGRADO dentro del recuadro dorado (John prefirió esta versión a la tarjeta hermana en el hero); BuyBox con DOS tarjetas (dorada y azul, separador «o», sin precio grande en la azul: «el 12 ya está en el botón»); CTA final con botón y sello. Botón «PAGAR CON CRIPTO — €12» en las tres puertas (mayúsculas por CSS). Un solo `<dialog>` y un solo script por página, compartidos por los tres botones (HTML 273 → 234 KB). Sello «Powered by NOWPayments» con 8 monedas (`CryptoCoinIcon.astro`; USDC como «$», BNB en disco de color). Landings hermanas byte a byte iguales.
- **Revisión adversarial (5 lentes + refutador, en serie por la térmica):** 31 hallazgos, 24 confirmados, 22 aplicados en `a989f39`. Quedan DOS para John: (1) la casilla «condiciones de compra y devoluciones» enlaza a `/terminos`, que no habla de productos digitales ni del desistimiento de 14 días → falta una sección legal propia; (2) «Continuar al pago» nace `disabled` hasta marcar las dos casillas: patrón habitual, pero sin explicación para lector de pantalla.
- **Decisión de John (03:15): «probamos primero el pago antes de replicar al resto de productos».** La réplica a las otras 3 plantillas + mega-pack (con `CRYPTO_PRODUCTS_EXCLUDE` para el eBook y el gate ampliado a las 46) la preparó un agente en el worktree `.claude/worktrees/agent-a0b26e4cbb20410c2` (rama `worktree-agent-a0b26e4cbb20410c2`, sobre `e2b9cae`; OJO: ese punto lleva el hero como tarjeta hermana, revertido después en `ad5bce5` → al aplicar el diff, mantener el hero integrado). NO está aplicada ni revisada.

## Siguiente sesión (sustituye a la anterior)
...
**Env en Netlify:** `CRYPTO_PRODUCTS_EXCLUDE=pro-prompts-ebook` en TODOS los contextos (scope builds+functions, fijada el 18-sep). `CRYPTO_PRODUCTS=all` en `deploy-preview` y, desde el 19-sep 00:2x, también en `production` (fijada ANTES del merge para que el build de `main` naciera con ella).

**Cierre (19-sep-2026, verificado en LIVE):** PR #81 fusionada (`7ebc64e`), deploy publicado; gate sin flags → 47 landings con botón / 1 sin él / 0 fallos; eBook → `403`. Revisión adversarial previa (40 agentes): 5 hallazgos confirmados y aplicados (`d7911d2`), 13 refutados. Handoff: `SESSION_HANDOFF_2026-09-18-cripto-replica-48.md`. Recordatorio: `netlify env:set` sobre una variable existente **conserva el scope** si solo se pasa `--context`.
