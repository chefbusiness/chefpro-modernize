...
| Guion `guias-v2_0/guion_guia_chocolateria_obrador.py` (5.569 líneas) + `guia-chocolateria/verificar_guion.py` | ✅ **verde**: 20 caps + anexo (`sin_numerar`), 37.900 palabras, `bloques` según D50 (2 en 09/10/11/12/16/19), 337 `C()`, 74 tablas, 458 puntos por epígrafe (21/21 exactos contra §4), 183 ids (13 de fiabilidad baja solo en `sector`); bonus 1 en **6 entradas** (11 tablas; desviación declarada de D50: 45 bloques, no 40) y bonus 2 en 12. Gates nuevos: ids «bare» de D44 y cifras de fiabilidad baja (probado inyectando 3 defectos). Cazó 3 tablas que imprimían euros como % (`es_fila_porcentual` mira las dos primeras celdas) | `261d28b` |
| Verificación independiente de los 23 fixes de los xlsx + **segunda pasada de fixer** | ✅ el verificador (sonnet, pycel) confirmó 4 altos y 8 medios, y cazó **1 sin aplicar, 5 a medias y un defecto NUEVO de un fix** (`%%` literal en 4 celdas de venta). Fixer 2: los 7 aplicados (B9 columna del 40 % de materias añadidas · B10 `%%` · A8 `_notas` fuera de los 9 mapas · A9 Title Case «sobre/tras/según/desde» · A10 `freeze_panes` en 4 y 6 · B4 «Inmovilizado amortizable» en `capex!Resumen!B23` · B8 `nota_legal` con artículo 183/201 + `EXCEPCIONES_SIN_ARTICULO` 14 ids + `gate_legal(exigir_articulo=True)`), generadores con `GUIA_BUILD_DIR`, regenerados tras el guion; **`gate_libros` 9/9, `verificar_guion` 0 rotas, md5 build = dl 9/9** | `a52b8a4`, este commit |
...
## 6. Qué queda (sesión B2+C — este producto se termina EN LOCAL por decisión de John; los siguientes, en el VPS)

1. **Redactores por bloque** (`guias-v2_0/dump_prompts.py` → 45 agentes Sonnet → `check_bloque.py`), como en Pastelería (47 bloques en 16 min).
2. `documentos.py` (pasar `--min-palabras-cap 1300` explícito) → guía + bonus 2 (PDF + DOCX) + business plan (DOCX, ≥9 `<w:tbl>`); paridad PDF↔DOCX; medir páginas.
3. Refutación de documentos (opus, tres lentes) → fixer sobre los `.txt` → reensamblar → censo.
4. Capa de producto (SPEC §8.1, 29 filas): 49 en `products-catalog.ts`, `payment-links.ts`, `product-prices.ts`, functions (14 claves de `PRODUCT_FILES`), zona app (`fase5-generate-zona-app.py`), landing `productos/guias/guia-chocolateria-obrador.ts` (sin tachado, sin testimonios, páginas MEDIDAS), hub ×2 con `comingSoon` **vaciado** + guarda `{comingSoon.length > 0 && …}`, changelog, `CRYPTO_PRODUCTS` (D16), 4 páginas `/usos/`, las **6 FAQ de consultoría** (D12), banners en 8 posts con `fase8x` generalizado (D46), imágenes (galería + OG).
5. Gates LIVE (SPEC §8.2): censo, paginas-gate, robots-gate, whatsapp-gate, gate-flujo-postpago `--only`, fase6-gate.
6. Broadcast de Resend: **24-oct** (o kit 24-oct + guía 29-oct si John aprueba D53), programable desde el 24-sep, siempre tras `GET /broadcasts`.

**Pendiente de John:** Payment Link + env var (datos en §10 de la SPEC y en el mensaje de las 18:16) · decisión sobre **D53** (kit 2.1, ≈0,15 M) · compra de prueba tras el LIVE · 5-10 min de voz (opcional) · si en algún momento quiere sembrar otro «Próximamente».

**Residuos conocidos, no bloqueantes:** libro 7 con `freeze_panes` en 9/11 hojas · 14 ids legales sin artículo componible (en `EXCEPCIONES_SIN_ARTICULO`, con motivo) · la nota-puente del cap. 12 sobra si se aprueba el kit 2.1 (retirar en v1.1) · «La Almendra» sin comprobar en la OEPM (registro inaccesible desde el Mac; sin negocio vivo en la web).
