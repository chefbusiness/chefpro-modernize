#!/bin/zsh
# Vigilante térmico: congela procesos pesados de agentes (python/node/esbuild/grep/rg) Y los daemons de macOS que recalientan el Mac (mediaanalysisd, photoanalysisd, bird) por encima de 66 °C y los reanuda por debajo de 63 °C.
LOG=$1; END=$((SECONDS+3600)); typeset -A FROZEN
while [ $SECONDS -lt $END ]; do
  T=$(istats cpu temp | grep -o '[0-9]*\.[0-9]*' | head -1)
  if [ "$(echo "$T >= 66" | bc)" = "1" ]; then
    ps -Ao %cpu,pid,comm | awk '$1>60' | grep -i -E "python|node|esbuild|grep|rg$|mediaanalysisd|photoanalysisd|photolibraryd|bird$" | while read cpu pid comm; do
