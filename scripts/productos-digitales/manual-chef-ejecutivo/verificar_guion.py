...
# -*- coding: utf-8 -*-
"""
verificar_guion.py — gate del GUION del «Manual del Chef Ejecutivo»
ANTES de escribir una sola palabra de prosa.

Clon parametrizado del gate del hermano (`manual-manager/verificar_guion.py`),
con el mismo juego de comprobaciones 1-9 y las rutas de este producto.

Lo que se comprueba (SPEC `manual-chef-ejecutivo-SPEC.md` §4, §4.2 y §6):
...
La frontera con el manual hermano (D11) NO se comprueba aquí: está en
`auditorias/manual-chef-ejecutivo-frontera-guion-2026-09-06.md`.
Via: Claude Code
"""
...
if not os.path.isdir(XLSX_DIR):
    falla(f'no existe {XLSX_DIR}: copia los xlsx de manual-chef-ejecutivo/build/ ahí')
    print('\n'.join(fallos))
    sys.exit(1)
