...
for (let i = 0; i < LENTES.length; i += PAR) {
  ...
  const r = await parallel(par.map((l) => () => agent(`REFUTADOR (${l.key}) del Kit Plan Financiero v2.0. Tu misión es DEMOSTRAR QUE NO ESTÁ LISTO. SPEC: ${SPEC}. R1 original: ${R1} (comprueba que cada id marcado «resuelto en §N» de la tabla final está de verdad resuelto en la copia; los §6/§7-bis no). Genera TU copia (\`CLAUDE_SCRATCHPAD=${SCR}/propio-ref-${l.key} python3 ${PKG}/main.py --dry-run --json ${OUT}/ref-${l.key}.json\`) y aplícale la lente: ${l.lente} Ids ${l.pref}-NN. Máx. 30 hallazgos, cada uno con evidencia (celda o línea REAL). ESCRIBE el JSON completo en ${AUD}/kit-plan-financiero-v2-ref-${l.key}.json y devuelve el schema. ${RULES}`,
...
const corr = await agent(`Corrector del Kit Plan Financiero v2.0. Lee ENTEROS los 3 ficheros ${AUD}/kit-plan-financiero-v2-ref-{dominio,excel,coherencia}.json (${totalRef} hallazgos en total: NO trabajes de memoria ni de un resumen). Para CADA id: o lo resuelves en ${PKG}/*.py / los .ts de integración (con celda/línea de prueba), o lo llevas a no_aplicados con motivo defendible (falso positivo con evidencia, o descartado por §6/§7-bis de la SPEC). Cruza al final: nº de ids de entrada = resueltos + no_aplicados (ninguno perdido). Dry-run completo tras corregir: exit 0, idempotencia 0. Informe también en ${AUD}/kit-plan-financiero-v2-correccion.json. ${RULES}`,
