export const meta = {
  name: 'kit-inventario-v2',
  description: 'Construcción del Kit de Inventario v2.0 según kit-inventario-v2-SPEC.md: motor + grupos A/B/C (dry-run) → integración → 3 refutadores → corrección → ronda 2 → crítico. SOLO dry-run: la ejecución real la hace el orquestador.',
  phases: [
...
const SPEC = `${REPO}/scripts/productos-digitales/kit-inventario-v2-SPEC.md`
const R1 = `${REPO}/scripts/productos-digitales/auditorias/kit-inventario-R1.json`
const PKG = `${REPO}/scripts/productos-digitales/kit-inventario-v2_0`
const DL = `${REPO}/astro-site/public/dl/kit-inventario`
const AUD = `${REPO}/scripts/productos-digitales/auditorias`
const OUT = `${SCR}/kit-inv-v2`
const MODELO = `${REPO}/scripts/productos-digitales/kit-escandallos-v2_0`
const RULES = `REGLAS DURAS: (1) PROHIBIDO modificar ${DL}/** — todo por \`CLAUDE_SCRATCHPAD=${SCR}/propio-<rol> python3 ${PKG}/main.py --dry-run [--solo ...] --json ${OUT}/<rol>.json\` (nunca KIT_INVENTARIO_APPLY). (2) REGLA TÉRMICA (el Mac se apaga a 65 °C): \`istats cpu temp\` antes de cada python; ≥ 60 °C → \`sleep 90\`; un python a la vez; sin builds/Playwright. (3) No git add/commit/stash/clean. (4) Cita fichero:hoja:celda. (5) Escribe tu código/JSON PRONTO en el repo y ve actualizándolo (el Mac se ha apagado de madrugada dos veces: lo no commiteado se pierde, y el orquestador commitea entre fases). (6) python3 3.7 / openpyxl 3.1.3: sin f-strings con '=' ni walrus; pycel NO implementa COUNTA ni MODE (la SPEC da los sustitutos). (7) Convenciones de familia: verdes E8F5E9 desbloqueados, IFERROR en toda división, parámetros en celda, inject_cache.py al final de main.py, idempotencia (2.ª pasada 0 diferencias), protección de hoja sin contraseña. (8) Modelo de paquete: ${MODELO}/ (motor.py, grupo_a/b/c.py, main.py con --dry-run/--solo/APPLY/respaldo): imita su estructura y sus gates.`
...
phase('Grupos')
const GRUPOS = [
  { rol: 'grupo_a', que: '§2 — 01-inventario-stock-diario (precio/valor, 50 filas precargadas REALES, stock mínimo + CF, Resumen Dashboard con SUMIFS), 02-fichas-proveedores (campos nuevos + 6 ejemplos realistas), BONUS-08-inventario-rapido-mensual (coherencia con 01)' },
  { rol: 'grupo_b', que: '§3 — 03-pedidos-compra (IVA por línea con desplegable y tipo por defecto por categoría, totales por tipo, hoja Proveedores VISIBLE de 20 filas + histórico si la SPEC lo manda), 04-recepcion-mercancias (nº lote, nº albarán, tabla LEGAL de temperaturas por familia, «Conforme» por fórmula + CF rojo, no conformidades), 05-control-mermas (precio y coste por línea, Análisis por Categoría y Dashboard con SUMIFS, Plan de Acción estructurado)' },
  { rol: 'grupo_c', que: '§4 — 06-fifo-caducidades (semáforo de 4 estados por fórmula con CADUCADO→RETIRAR, vida útil tras apertura, protocolo coherente), 07-analisis-costes-compras (ventas y cubiertos en verdes, Dashboard KPIs con fórmulas reales, Top 20 como tabla de entrada con LARGE/INDEX/MATCH — decisión §7-bis.1), BONUS-09-calculadora-punto-pedido (EOQ parametrizada en celdas y capada por vida útil y consumo, stock de seguridad en UNIDADES con ayuda días×consumo, Parámetros en la misma unidad, sin promesa de «simulador» — §7-bis.4)' },
]
const conts = []
for (let i = 0; i < GRUPOS.length; i += PAR) {
  const par = GRUPOS.slice(i, i + PAR)
  const r = await parallel(par.map((g) => () => agent(`Constructor del ${g.rol} del Kit de Inventario v2.0. SPEC: ${SPEC} (lee ENTERA; tu parte: ${g.que}; respeta §6 descartes y §7-bis). R1: ${R1} (tus ids). Motor ya escrito: ${PKG}/motor.py y ${PKG}/main.py (léelos y usa sus utilidades; no dupliques). Ficheros reales en ${DL}/ — ábrelos ANTES de escribir. Escribe ${PKG}/${g.rol}.py exponiendo lo que main.py espere (compruébalo en main.py). Los datos precargados que la SPEC pida deben ser REALES del oficio (categorías/unidades/precios coherentes producto a producto: nada de rotaciones cíclicas). Prueba: \`CLAUDE_SCRATCHPAD=${SCR}/propio-${g.rol} python3 ${PKG}/main.py --dry-run --solo motor,${g.rol} --json ${OUT}/${g.rol}.json\` → exit 0, idempotencia 0; demuestra con pycel los cálculos nuevos de tus ficheros (cambia inputs y comprueba la dirección del resultado); cita cada celda. Informe también en ${AUD}/kit-inventario-v2-${g.rol}.json. ${RULES}`,
...
phase('Integración')
const integ = await agent(`Integrador del Kit de Inventario v2.0 (§5 de ${SPEC}, con §7-bis.3/5/6). Con la copia dry-run completa (genera una: \`CLAUDE_SCRATCHPAD=${SCR}/propio-integ python3 ${PKG}/main.py --dry-run --json ${OUT}/integ.json\`): actualiza ${REPO}/astro-site/src/data/productos/kits/kit-inventario.ts (líneas que la SPEC cita: interconexión→coherencia, APPCC matizado, 10 categorías, cifras reales medidas en la copia, «50-100 €/mes», badge §7-bis.6, FAQ licencia §7-bis.5, updateNote 2.0 — NO tocar aggregateRating/reviews/testimonios/precios), ${REPO}/src/pages/KitInventarioDashboard.tsx (tarjetas coherentes con lo que hay), entrada 'kit-inventario' de ${REPO}/src/data/productos-changelog.ts (version 2.0, updated 2026-08-23, entrada encima de la 1.1 con 10-13 cambios REALES en lenguaje de cliente) y ${REPO}/netlify/functions/resend-access.ts si cita cifras. Sintaxis con typescript.transpileModule (node_modules tiene typescript); nada de builds. Devuelve el schema (en pruebas: cada cifra publicada con su medida). ${RULES}`,
...
for (let i = 0; i < LENTES.length; i += PAR) {
  ...
  const r = await parallel(par.map((l) => () => agent(`REFUTADOR (${l.key}) del Kit de Inventario v2.0. Tu misión es DEMOSTRAR QUE NO ESTÁ LISTO. SPEC: ${SPEC}. R1 original: ${R1} (comprueba que cada id marcado «resuelto en §N» de la tabla final está de verdad resuelto en la copia; los §6/§7-bis no). Genera TU copia (\`CLAUDE_SCRATCHPAD=${SCR}/propio-ref-${l.key} python3 ${PKG}/main.py --dry-run --json ${OUT}/ref-${l.key}.json\`) y aplícale la lente: ${l.lente} Ids ${l.pref}-NN. Máx. 30 hallazgos, cada uno con evidencia (celda o línea REAL). ESCRIBE el JSON completo en ${AUD}/kit-inventario-v2-ref-${l.key}.json y devuelve el schema. ${RULES}`,
...
const corr = await agent(`Corrector del Kit de Inventario v2.0. Lee ENTEROS los 3 ficheros ${AUD}/kit-inventario-v2-ref-{dominio,excel,coherencia}.json (${totalRef} hallazgos en total: NO trabajes de memoria ni de un resumen). Para CADA id: o lo resuelves en ${PKG}/*.py / los .ts de integración (con celda/línea de prueba), o lo llevas a no_aplicados con motivo defendible (falso positivo con evidencia, o descartado por §6/§7-bis de la SPEC). Cruza al final: nº de ids de entrada = resueltos + no_aplicados (ninguno perdido). Dry-run completo tras corregir: exit 0, idempotencia 0. Informe también en ${AUD}/kit-inventario-v2-correccion.json. ${RULES}`,
...
const r2 = await agent(`Verificador de RONDA 2 del Kit de Inventario v2.0. Lee los 3 ref-*.json y ${AUD}/kit-inventario-v2-correccion.json. Regenera TU copia (dry-run completo) y comprueba UNO A UNO cada id de los refutadores: resuelto de verdad (celda) / mal resuelto (evidencia) / no_aplicado con motivo que se sostiene o no. Gates: exit, idempotencia, censo --fail, data_only sin fallos, transpileModule de los .ts editados. Además abre el kit como cliente recién pagado: 5 minutos por fichero, hallazgos nuevos máx. 8. listo=true SOLO si 0 mal_resueltos, 0 injustificados, gates limpios y 0 hallazgos nuevos altos. ESCRIBE ${AUD}/kit-inventario-v2-ronda2.json y devuelve el schema. ${RULES}`,
...
const crit = await agent(`Crítico de completitud del Kit de Inventario v2.0. Lee ${AUD}/kit-inventario-v2-ronda2.json, la corrección, los 3 ref-*.json, ${AUD}/kit-inventario-v2-motor.json y los de grupos, y la tabla id→sección de ${SPEC} cruzada contra la corrección (¿algún id del R1 se perdió por el camino?). Verifica en SOLO LECTURA 5 celdas clave de la última copia dry-run. listo_para_real, bloqueos con celda, cifras para la capa de producto (las que publicará la landing, medidas), orden_ejecucion_real (comandos exactos para el orquestador: apply, censo, gate offline, commit, LIVE). Escribe ${AUD}/kit-inventario-v2-critico.json. ${RULES}`,
