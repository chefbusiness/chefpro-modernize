...
Genera, para los 44 productos del registro astro-site/src/lib/zona-app.ts:
  - 44 páginas  astro-site/src/pages/<accessPath>.astro   (S1)
  - 44 wrappers astro-site/src/islands/library/<X>LibraryIsland.tsx (S2)
  - 44 páginas  astro-site/src/pages/<libraryPath>.astro  (S2)
...
Cross-checks (el script ABORTA si fallan):
  - 44 entradas en el registro; ficheros de gate y dashboard existen.
  - storageKey de App.tsx == storageKey del registro (44/44).
...
def main():
    ...
    if len(entries) != 44:
        fail(f"registro: {len(entries)} entradas parseadas (esperado 44)")
    ...
    if CHECK:
        ...
        print("✅ --check: 132 ficheros generables coinciden byte a byte con el disco")
    else:
        print(f"✅ Generados/actualizados {len(changed)} ficheros "
              f"(44 access + 44 islands + 44 library = 132 gestionados)")
        for c in changed:
            print(f"   {c}")
