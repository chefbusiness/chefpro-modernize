...
Genera, para los 49 productos del registro astro-site/src/lib/zona-app.ts:
  - 49 páginas  astro-site/src/pages/<accessPath>.astro   (S1)
  - 49 wrappers astro-site/src/islands/library/<X>LibraryIsland.tsx (S2)
  - 49 páginas  astro-site/src/pages/<libraryPath>.astro  (S2)
...
Cross-checks (el script ABORTA si fallan):
  - 49 entradas en el registro; ficheros de gate y dashboard existen.
  - storageKey de App.tsx == storageKey del registro (49/49).
...
def main():
    ...
    if len(entries) != 49:
        fail(f"registro: {len(entries)} entradas parseadas (esperado 49)")
    ...
    if CHECK:
        ...
        print("✅ --check: 147 ficheros generables coinciden byte a byte con el disco")
    else:
        print(f"✅ Generados/actualizados {len(changed)} ficheros "
              f"(49 access + 49 islands + 49 library = 147 gestionados)")
        for c in changed:
            print(f"   {c}")
