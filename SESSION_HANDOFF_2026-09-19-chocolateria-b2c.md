...
## 5. Cómo retomar — siguiente sesión (PAR): «Tareas Recurrentes: Taquería Mexicana», primer producto del top-20

- **Dónde**: en el **VPS** (decisión de John del 12-sep: los productos siguientes fuera del Mac). Allí no aplica la regla térmica y `bridge.py` vive en `/root/chefbusiness-ai/` (aunque los productos NO usan bridge: subagentes Anthropic). Claves en su `.env`.
- **Decisión previa de John**: ¿el **Mega Pack** (89 €, anunciado como «13 kits» con 19 vivos) pasa a incluir los kits nuevos? Condiciona la landing del Mega Pack y el copy del kit nuevo.
- **Método**: réplica de 1 sesión con el motor **Kit de Tareas v2.4** (familia `scripts/productos-digitales/kit-tareas-v2_0/`, módulo de contenido por kit `contenido_kit_tareas_<pid>.py`): research (DataForSEO ya con saldo: «taquería mexicana» 210/mes ES; `--pais`/`--idioma` para MX si se mide LATAM) + SERP → SPEC corta → contenido → generador → gates (`censo-entregables --fail`, `gate-no-latinos`, `postprocess-transversal`) → capa de producto calcando `8457077`/PR #84 (nace con Stripe + NOWPayments, `zona-app.ts`, `product-prices.ts`, 3 puertas cripto) → PR con preview → Payment Link de John → merge → gates LIVE → broadcast en la cola de Resend (siguiente hueco tras el 24/29-oct).
- **Comprobaciones baratas que hoy ahorraron horas**: medir tildes del fichero de datos contra un hermano antes de redactar; leer `erratas_reparadas` del informe; la ronda 2 con lector de regresiones; nunca más de 2 rondas.
- Quitar su tarjeta de `comingSoon` en los DOS ficheros del hub al publicar (22 → 21).
