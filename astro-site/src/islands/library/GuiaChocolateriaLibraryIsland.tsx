/**
 * Island de /guia-chocolateria-obrador-library (Fase 5, GENERADO por scripts/astro-migration/
 * fase5-generate-zona-app.py — editar el generador, no este fichero).
 * Réplica exacta de la composición de la SPA para esta ruta en App.tsx:
 * storageKey/redirectTo extraídos VERBATIM de App.tsx por el generador.
 */
import ProtectedRoute from '../../../../src/components/shared/ProtectedRoute';
import GuiaChocolateriaDashboard from '../../../../src/pages/GuiaChocolateriaDashboard';

export default function GuiaChocolateriaLibraryIsland() {
  return (
    <ProtectedRoute storageKey="guia-chocolateria-obrador-jwt" redirectTo="/guia-chocolateria-obrador">
      <GuiaChocolateriaDashboard />
    </ProtectedRoute>
  );
}
