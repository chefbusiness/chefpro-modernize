/**
 * Island de /kit-tareas-chocolateria-library (Fase 5, GENERADO por scripts/astro-migration/
 * fase5-generate-zona-app.py — editar el generador, no este fichero).
 * Réplica exacta de la composición de la SPA para esta ruta en App.tsx:
 * storageKey/redirectTo extraídos VERBATIM de App.tsx por el generador.
 */
import ProtectedRoute from '../../../../src/components/shared/ProtectedRoute';
import KitTareasChocolateriaDashboard from '../../../../src/pages/KitTareasChocolateriaDashboard';

export default function KitTareasChocolateriaLibraryIsland() {
  return (
    <ProtectedRoute storageKey="kit-tareas-chocolateria-jwt" redirectTo="/kit-tareas-chocolateria">
      <KitTareasChocolateriaDashboard />
    </ProtectedRoute>
  );
}
