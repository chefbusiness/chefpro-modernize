...
- **⚠️ En móvil, CUALQUIER elemento que sobresalga del ancho de pantalla desplaza la página entera** (Android/iOS ensanchan el «layout viewport»; el escritorio no, así que en el Chrome de Windows no se ve). Cazado por John el 21-sep-2026 en el blog: la cabecera sumaba **431 px en un móvil de 360** (contenedor con 2rem de margen + logo de 206 px + selector de idioma + hamburguesa) y la tabla de 4 columnas del post del hito medía 466 px. Arreglo en tres capas: `Header.astro`/`ModernHeader.tsx` con `px-4 sm:px-8` y logo `h-6 sm:h-8`; `html { overflow-x: hidden }` en `global.css` (sólo en `html`: en `body` rompe el `sticky` de la cabecera); y en `BlogPost.astro` las tablas de Markdown pasan a bloque con scroll propio por debajo de 768 px. **Cómo medirlo sin Playwright**: en el Chrome de Windows, `document.documentElement.style.width="360px"` y listar los elementos cuyo `getBoundingClientRect().right` supera 360 (la ventana no baja de 628 px y el sitio no se deja meter en un iframe).
