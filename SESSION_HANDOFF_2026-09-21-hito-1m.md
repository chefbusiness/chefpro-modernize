...
## 8. Scroll lateral en Android (PR #92, en producción)

John lo vio en el móvil (fotos): la página entera desplazada con un hueco blanco a la derecha, en el post, en el índice del blog y en otros posts. Medido con el Chrome de Windows emulando 360/390 px (`documentElement.style.width`): la **cabecera sumaba 431 px a 360** (2rem de margen del contenedor + logo de 206 px + selector de idioma + hamburguesa), la **tabla de 4 columnas del post medía 466 px** y el anillo animado del botón de WhatsApp sobresale 4 px. En escritorio nada de eso desplaza la página; en Android/iOS ensancha el «layout viewport». Tres capas: cabecera `px-4 sm:px-8` + logo `h-6 sm:h-8` (a 360 px queda en 344), `html { overflow-x: hidden }` en `global.css` (sólo en `html`) y tablas de Markdown en bloque con scroll propio por debajo de 768 px. Verificado en el preview (cabecera dentro del contenedor a 360/390, `overflow-x` de `html` = hidden, tablas `display:block` con `overflow-x:auto`) y en producción. **Pendiente: confirmación de John en Android.**
